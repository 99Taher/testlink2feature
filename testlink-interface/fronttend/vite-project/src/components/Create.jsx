...
const Create = ({ projects, onSuccess, onCancel }) => {
  ...
  // Destructuration
  const {
    step, projectId, projectName, actionType, 
    suiteName, suiteDescription, existingSuites,
    selectedSuiteId, testCases, loading, error, formErrors
  } = state;

  // Effet pour charger les suites
  useEffect(() => {
  ...
  try {
    ...
    setState(prev => ({ ...prev, existingSuites: response.data }));
  } catch (err) {
    setState(prev => ({
      ...prev,
  ...
  // Création des test cases
  const createTestCases = async () => {
  try {
    const suiteInfo = existingSuites.find(s => s.id.toString() === selectedSuiteId);
    
    if (!suiteInfo) {
      throw new Error("Information de la suite introuvable");
    }
    ...
    const results = await Promise.all(
      testCases.map(tc => 
        axios.post('http://localhost:4000/api/create/testcase', {
          ...
          testcasename: tc.name,  // Nom du champ corrigé
          summary: tc.summary || '',
    ...
    setState(prev => ({
      ...prev,
      error: `Erreur création test case: ${err.response?.data?.error || err.message}`
    }));
    throw err;
  }
};
    ...
    setState(prev => ({
      ...
      testCases: [
        ...prev.testCases,
        {
          ...
          summary: '',
          steps: [{
            stepNumber: prev.testCases.length + 1,
            actions: '',
            expectedResults: '',
            executionType: 1
          }]
        ...
        <div className="create-step">
          ...
          {actionType && (
            <div className="form-actions">
              <button
                type="button"
                onClick={() => setState(prev => ({ ...prev, actionType: '', projectId: '' }))}
                disabled={loading}
              >
                Annuler
              </button>
              <button
                ...
                disabled={!actionType || loading || !!formErrors.project}
              >
                Continuer
              </button>
            </div>
